    ...
    <div className="ew-shell" style={sx('display:flex;height:100%')}>
      <aside className="ew-sidebar" style={sx('width:218px;flex:none;display:flex;flex-direction:column;padding:16px 12px 14px;position:relative;background:linear-gradient(180deg,#121813,#0C100D);box-shadow:inset -1px 0 0 rgba(237,235,226,.09)')}>
        ...
        <nav className="ew-sidebar-nav" style={sx('display:flex;flex-direction:column;gap:2px')}>
          {vm.navItems.map((n, i) => (
            <button key={i} onClick={n.go} style={sx(n.style)} title={n.label}>
              <i className={'ph ' + n.icon} style={sx('font-size:16px')}></i><span className="ew-navlabel">{n.label}</span>
            </button>
          ))}
          <div className="ew-admin-divider" style={sx('display:flex;align-items:center;gap:8px;padding:16px 10px 6px')}>
            <span className="ew-navlabel" style={sx('font-size:9px;letter-spacing:.16em;text-transform:uppercase;color:rgba(237,235,226,.4)')}>Admin</span>
        ...
        <div style={sx('margin-top:auto;display:flex;flex-direction:column;gap:10px')}>
          <div className="ew-sync-indicator" style={sx('display:inline-flex;align-items:center;gap:7px;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:rgba(237,235,226,.45);padding:0 8px;white-space:nowrap')}>
            <span style={sx('width:6px;height:6px;border-radius:50%;background:var(--color-accent);box-shadow:0 0 6px var(--color-accent)')}></span>
            <span className="ew-navlabel">Comp sync 14m ago</span>
          </div>
          {vm.userMenu && (
      ...
      <div style={sx('flex:1;min-width:0;display:flex;flex-direction:column')}>
        ...
        <header style={sx('flex:none;display:flex;align-items:center;gap:14px;padding:12px 24px;border-bottom:1px solid var(--color-divider);background:var(--color-surface);flex-wrap:wrap')}>
          <h4 style={sx('margin:0;font-size:16px;letter-spacing:-0.01em')}>{vm.pageTitle}</h4>
          <span className="ew-topbar-meta" style={sx('font-size:12px;color:var(--color-neutral-500)')}>{vm.dealerName}</span>
          <span style={sx('margin-left:auto;display:inline-flex;align-items:center;gap:7px;font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--color-neutral-500)')}>
            <span style={sx('width:6px;height:6px;border-radius:50%;background:var(--color-accent);box-shadow:0 0 6px var(--color-accent)')}></span><span className="ew-topbar-meta">Market feed live</span>
          </span>
        </header>
